<x-layout>
		{{-- 
        x-slot:title mengirimkan data variable $title ke dalam <title> 
        yang ada di file layout utama (resources/views/components/layout.blade.php) 
    --}}
		...
		<div class="py-4 px-4 mx-auto max-w-screen-xl lg:px-6">
				<div class="mx-auto max-w-screen-md sm:text-center">
						...
						{{-- 
                FORM PENCARIAN
                Secara default form akan mengirim data ke URL yang sama (method="GET").
                Hasilnya URL akan menjadi: /posts?search=katakunci
            --}}
						<form>
								{{-- 
                    LOGIKA PENTING: MEMPERTAHANKAN CATEGORY SAAT SEARCH
                    Jika di URL sedang ada ?category=lifestyle, lalu user melakukan pencarian,
                    input hidden ini memastikan 'category' tetap terkirim bersama 'search'.
                    Hasil URL gabungan: /posts?category=lifestyle&search=katakunci
                --}}
								@if (request('category'))
		...
		<div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-8 lg:px-0">
				<div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
						{{-- LOOPING POSTS: Iterasi array $posts yang dikirim dari Route/Controller --}}
						@forelse ($posts as $post)
								<article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
										<div class="flex justify-between items-center mb-5 text-gray-500">
												{{-- 
                            DYNAMIC LINK
                            Mengarahkan ke halaman posts tapi difilter berdasarkan slug category 
                        --}}
												<a href="/posts?category={{ $post->category->slug }}" wire:navigate>
														{{-- 
                                DYNAMIC CLASS (Tailwind)
                                Mengambil warna dari database ($post->category->color). 
                                Pastikan di css/safelist.txt sudah ditambahkan semua kemungkinan warna.
                            --}}
														<span
												...
												{{-- diffForHumans(): Mengubah timestamp jadi format "2 hours ago" atau "1 week ago" --}}
										...
										{{-- Str::limit: Memotong text body jadi 150 karakter + '...' agar tampilan rapi --}}
										<p class="mb-5 font-light text-gray-500 dark:text-gray-400">{{ Str::limit($post->body, 150) }}</p>

										<div class="flex justify-between items-center">
												{{-- AUTHOR RELATIONSHIP: Mengambil data user (author) dari relasi post --}}
												<a href="/posts?author={{ $post->author->username }}" wire:navigate>
						...
						@empty
								<div class="col-span-3 flex flex-col items-center space-y-4">
										...
										<a wire:navigate href="/posts" class="text-blue-600 hover:underline">&laquo;Back to posts</a>
								</div>
						@endforelse
				</div>
		</div>
